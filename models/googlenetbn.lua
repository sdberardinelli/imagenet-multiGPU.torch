local function inception(input_size, config)
   ...
   if config[4][2] ~= 0 then
      ...
      pool:add(nn.SpatialBatchNormalization(config[4][2],1e-3,nil,false))
   end
   concat:add(pool)
...
function createModel(nGPU)
   ...
   features:add(nn.SpatialBatchNormalization(64,1e-3,nil,false))
   ...
   features:add(nn.SpatialBatchNormalization(192,1e-3,nil,false))
   features:add(nn.SpatialMaxPooling(3,3,2,2):ceil())
   ...
   features:add(nn.SpatialBatchNormalization(576,1e-3,nil,false))
   ...
   main_branch:add(nn.SpatialBatchNormalization(1024,1e-3,nil,false))
   ...
   main_branch:add(nn.BatchNormalization(1024, 1e-3,nil,false))
   ...
   aux_classifier:add(nn.SpatialBatchNormalization(128,1e-3,nil,false))
   aux_classifier:add(nn.View(128*4*4):setNumInputDims(3))
   aux_classifier:add(nn.Linear(128*4*4,768))
   aux_classifier:add(nn.BatchNormalization(768, 1e-3,nil,false))
